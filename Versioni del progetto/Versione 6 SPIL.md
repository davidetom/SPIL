...
- $x^*_{ru}$: Frequenza ideale di ritiro settimanale per il rifiuto $r$ e l'utente $u$.
...
- $t^{viaggio}_{ab}$: Tempo di viaggio dal nodo $a$ al nodo $b$.

- $t^{carico}_{rt_u}$: Tempo di carico per unità (svuotamento mastelli) presso l'utente $u$ per il rifiuto $r$.
...
- $cd, cm$: Costo unitario per distanza percorsa e costo orario della manodopera.
...
Il tempo di attività totale del veicolo $v$ è dato dalla somma del tempo di viaggio su tutti gli archi del routing $P_{v}$ e del tempo Tdi carico per tutti i mastelli degli utenti del routing $P_{v}$.

$$T_{v} = \sum_{(a,b) \in P_{v}} t^{viaggio}_{ab} + \sum_{u \in P_{v}} \left( t^{carico}_{rt_u} \cdot Q_{ru}(X_r) \right)$$
