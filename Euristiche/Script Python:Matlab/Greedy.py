...
def build_routes(data: dict, r: str, X_r: float, gamma: float = 0.5, k_scala: float = 1.0) -> dict:
    ...
    # Inizializziamo f_partial applicando subito la scalatura F_tot per l'insoddisfazione
    f_partial = gamma * (F_insoddis * k_scala)
    first_seed = feasible_seeds[0]
    unassigned.discard(first_seed)
    open_vehicles.append(open_new_vehicle(first_seed))
    
    # Costi logistici scalati per (1 - gamma)
    f_partial += (1 - gamma) * (c_fixed_r * X_r)
    ...
    while unassigned:
        ...
        for idx, veh in enumerate(open_vehicles):
            ...
            for i in sorted_candidates_idx:
                ...
                # Manteniamo la coerenza dell'obiettivo: delta costo logistico pesato per (1 - gamma)
                dc_real = float(dcs[i]) * (1 - gamma)
        ...
        if next_seed is not None:
            ...
            # Costi logistici scalati per (1 - gamma)
            fo_new = f_partial + (1 - gamma) * (c_fixed_r * X_r + seed_cost)
        ...
        if action == "insert":
            ...
            f_partial += (1 - gamma) * (cd * dist_np[cur, chosen_u] + cm * (time_np[cur, chosen_u] + TC_arr[chosen_u]))
        else:
            new_veh = open_new_vehicle(chosen_u)
            f_partial += (1 - gamma) * (c_fixed_r * X_r + cd * dist_np[0, chosen_u] + cm * (time_np[0, chosen_u] + TC_arr[chosen_u]))
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_X):
                ...
                if result["F_total"] < best_total:
                    ...
                    best_X_r = result["X_r"]
                    best_F = {k: result[k] for k in ("F_total", "F_insoddis", "F_costo_fisso", "F_viaggio", "F_lavoro", "sat_fisica", "sat_tempo")}
